...
```bash
uv tool install --from "git+https://<COMPANY_GITLAB_URL>/steven.yang/trac-mcp-server.git" trac-mcp-server  # 替換為你自己的 fork（若已 fork）
```
...
每台新電腦只需：① 安裝 uv ② `uv tool install --from "git+https://<COMPANY_GITLAB_URL>/steven.yang/trac-mcp-server.git" trac-mcp-server` ③ 編輯 opencode.json 加入 trac 設定 ④ 重啟 OpenCode。<!-- 可替換為你自已的 trac-mcp-server fork -->
