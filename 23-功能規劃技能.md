...
1. 選擇安裝層級：
   - **全域（推薦，所有專案可用）**：`~/.agents/skills/project-plan-feature/`
   - **專案（僅當前專案）**：`.trae/skills/project-plan-feature/` 或 `.agents/skills/project-plan-feature/`

   ```bash
   # 全域
   mkdir -p ~/.agents/skills/project-plan-feature/
   # 或專案
   mkdir -p .trae/skills/project-plan-feature/
   ```
2. 從本 repo 的 `skills/23-project-plan-feature/SKILL.md` 複製內容到對應目錄的 `SKILL.md`
3. 重新載入 Trae（Cmd+R / Ctrl+R）
4. 對 Trae 說「規劃新功能」，確認可載入

### 在 Trae 上更新

替換對應目錄中的 `SKILL.md` 內容即可：
- 全域：`~/.agents/skills/project-plan-feature/SKILL.md`
- 專案：`.trae/skills/project-plan-feature/SKILL.md` 或 `.agents/skills/project-plan-feature/SKILL.md`

### 在 Trae 上移除

```bash
# 全域
rm -rf ~/.agents/skills/project-plan-feature/
# 或專案
rm -rf .trae/skills/project-plan-feature/
```
